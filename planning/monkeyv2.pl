...
at(ramp_goal,[Xb,Yb]) :- (at(banana,[Xba,Yba]), box_direction(DirB)),
...
climbup(monkey,[X1, Y1], Dir) :- 
			(at(box,[Xbo,Ybo]), at(monkey,[Xmo,Ymo]), at(ramp_goal,[Xra,Yra]), at(banana,[Xba,Yba]), 
...
climbable :- at(box,[Xbo,Ybo]), at(ramp_goal,[Xba,Yba]), Xbo is Xba, Ybo is Yba.


%goal will be location which places box between monkey and banana
%this allows monkey to push box towards banana
goal(X1, Y1) :- (at(box,[Xbo,Ybo]), at(ramp_goal,[Xb,Yb])), 
...
climb_goal(X1, Y1) :- (at(box,[Xbo,Ybo]), at(ramp_goal,[Xb,Yb]), 
...
pushto(monkey, box,[Xm1, Ym1],[Xbo1, Ybo1], Dir) :-
       (at(monkey,[Xm0, Ym0]),at(box,[Xbo0, Ybo0]), at(ramp_goal,[Xba, Yba])),
